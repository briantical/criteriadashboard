...
export class Cakes extends Component {
  ...
  getCakes = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_URL}/api/v1/cake/`, { headers })
      .then(response => {
  ...
  getCategories = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_URL}/api/v1/category/`, { headers })
      .then(response => {
        ...
        {spinner ? (
          ...
          cakes.map(cake => (
            <Tiles
              ...
              editCake={this.editCake}
            />
          ))
        )}
        <div className="addCake" onClick={this.showAddModal}>
          +
        </div>
      </div>
    );
