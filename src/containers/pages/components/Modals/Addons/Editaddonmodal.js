...
export class Editaddonmodal extends Component {
    ...
    handleSubmit = (event) =>{
        ...
        let {modalprops:{addon:{name,description,category,image,cost}}}= this.props;
        ...
        const {hideModal,modalprops:{editAddon,addon:{_id}}} = this.props;

        editAddon(_id,{name, category,description,image, cost});
        hideModal()
      };

    render() {
        const {categories,hideModal,modalprops:{addon:{name,description,category,image,cost}}} =this.props;
        const { errorMessage, spinner } =this.props;
        
        return (
            <div className="editaddonmodal" onClick={hideModal}>
...
export default connect(mapStateToProps, mapDispatchToProps)(Editaddonmodal);